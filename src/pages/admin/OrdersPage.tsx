...
export default function OrdersPage() {
    ...
    return (
        <div className="space-y-6 h-full flex flex-col">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto relative z-10">
                    ...
                    <div className="relative flex-1 sm:w-72">
                        ...
                        <input
                            ...
                            placeholder="N°, nom, table..."
                            className="w-full h-10 md:h-12 pl-10 md:pl-11 pr-4 rounded-2xl border border-stone-200 focus:border-orange-500 focus:ring-4 focus:ring-orange-50 text-xs md:text-sm font-medium transition-all"
                        />
                    </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center px-1">
                <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0 no-scrollbar w-full sm:w-auto">
                    <button
                        ...
                        className={`whitespace-nowrap px-3 md:px-5 py-2 md:py-2.5 rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-wider transition-all shadow-sm ${typeFilter === 'all' ? 'bg-stone-900 text-white translate-y-[-2px]' : 'bg-white text-stone-400 border border-stone-100 hover:border-stone-300'}`}
                    >
                        Tous ({data?.data?.length || 0})
                    </button>
                    <button
                        onClick={() => setTypeFilter('dine_in')}
                        className={`whitespace-nowrap px-3 md:px-5 py-2 md:py-2.5 rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-wider transition-all shadow-sm ${typeFilter === 'dine_in' ? 'bg-orange-500 text-white translate-y-[-2px]' : 'bg-white text-stone-400 border border-stone-100 hover:border-stone-300'}`}
                    >
                        🍽️ Salle ({data?.data?.filter(o => o.type === 'dine_in').length || 0})
                    </button>
                    <button
                        onClick={() => setTypeFilter('takeout')}
                        className={`whitespace-nowrap px-3 md:px-5 py-2 md:py-2.5 rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-wider transition-all shadow-sm ${typeFilter === 'takeout' ? 'bg-indigo-500 text-white translate-y-[-2px]' : 'bg-white text-stone-400 border border-stone-100 hover:border-stone-300'}`}
                    >
            ...
            {/* List View */}
            {viewMode === 'list' && (
                <div className="bg-white rounded-[2rem] border border-stone-100 shadow-sm flex-1 min-h-0 relative overflow-hidden">
                    <div className="overflow-x-auto h-full no-scrollbar">
                        <table className="w-full text-left border-collapse min-w-[800px]">
                            <thead className="bg-stone-50/50 sticky top-0 z-10 backdrop-blur-md">
                                <tr className="border-b border-stone-100">
                                    <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-stone-400">N°</th>
                                    <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-stone-400">Type</th>
                                    <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-stone-400">Référence / Table</th>
                                    <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-stone-400">Articles</th>
                                    <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-stone-400">Total</th>
                                    <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-stone-400">Statut</th>
                                    <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-stone-400">Heure</th>
                                    <th className="py-4 px-6 text-right text-[10px] font-black uppercase tracking-widest text-stone-400">Actions</th>
                                </tr>
                            </thead>
                                    ...
                                    .map(order => (
                                        <tr key={order.id} className="hover:bg-stone-50/50 transition-colors group">
                                            <td className="py-4 px-6">
                                                <span className="font-black text-stone-900 group-hover:text-orange-600 transition-colors">#{order.dailyNumber}</span>
                                            </td>
                                            <td className="py-4 px-6">
                                                <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${order.type === 'dine_in' ? 'bg-orange-50 text-orange-600' :
                                                    order.type === 'takeout' ? 'bg-indigo-50 text-indigo-600' :
                                                        'bg-blue-50 text-blue-600'
                                                    }`}>
                                                    {order.type === 'dine_in' ? 'Salle' : order.type === 'takeout' ? 'Emporter' : 'Livraison'}
                                                </span>
                                            </td>
                                            <td className="py-4 px-6">
                                                ...
                                                {order.pickupCode && <div className="text-[10px] font-mono text-stone-400 mt-0.5">{order.pickupCode}</div>}
                                            </td>
                                            <td className="py-4 px-6">
                                                <span className="text-stone-500 text-sm font-medium">{order.items.length} prod.</span>
                                            </td>
                                            <td className="py-4 px-6 transform transition-transform group-hover:scale-105 origin-left">
                                                <div className="font-black text-stone-900 text-sm">{order.formattedTotal || formatCurrency(order.totalAmount)}</div>
                                            </td>
                                            <td className="py-4 px-6">
                                                <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${order.status === 'pending' ? 'bg-yellow-50 text-yellow-700' :
                                                    order.status === 'in_progress' ? 'bg-orange-50 text-orange-600' :
                                                        order.status === 'delivered' ? 'bg-green-50 text-green-600' :
                                                            order.status === 'paid' ? 'bg-stone-100 text-stone-500' :
                                            ...
                                            <td className="py-4 px-6">
                                                <div className="text-stone-400 text-[11px] font-bold">
                                            ...
                                            <td className="py-4 px-6 text-right">
                                                <div className="flex justify-end gap-1">
                                                    {order.status === 'pending' && (
                                                        <>
                                                            <button
                                                                ...
                                                                className="bg-stone-900 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-stone-800 transition-all active:scale-95 shadow-sm"
                                                            >
                                                                Lancer
                                                            </button>
                                                            <button
                                                                onClick={() => handleStatusChange(order.id, 'cancelled')}
                                                                className="text-red-400 hover:text-red-500 p-1.5 rounded-xl hover:bg-red-50 transition-all"
                                                            >
                                                    ...
                                                    {order.status === 'in_progress' && (
                                                        <>
                                                            <button
                                                                ...
                                                                className="bg-orange-500 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-orange-600 transition-all active:scale-95 shadow-lg shadow-orange-100"
                                                            >
                                                                Servir
                                                            </button>
                                                        </>
                                                        ...
                                                        <div className="flex gap-1">
                                                            {order.type === 'takeout' && (
                                                                <button
                                                                    ...
                                                                    className="bg-green-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-green-700 shadow-lg shadow-green-100"
                                                                >
                                                                    Encaisser
                                                                </button>
                                                            )}
                                                            {order.type === 'delivery' && !order.deliveryStatus?.includes('picked') && (
                                                                <button
                                                                    ...
                                                                    className="bg-blue-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-blue-700 shadow-lg shadow-blue-100"
                                                                >
                                                                    Confier
                                                                </button>
                                                            )}
                                                    ...
                                                    {order.status === 'paid' && (
                                                        <button
                                                            onClick={() => handlePayAndPrint(order)}
                                                            className="bg-stone-100 text-stone-500 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-stone-200 transition-all"
                                                        >
                                                            Ticket
                                                        </button>
                                                    )}
                                    ...
                                    <tr>
                                        <td colSpan={8} className="py-20 text-center">
                                            <div className="flex flex-col items-center opacity-20">
                                                ...
                                                <p className="font-black uppercase tracking-widest text-sm">Cuisine Vide</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                            ...
                            <div className="bg-white p-6 rounded-2xl border-2 border-stone-100 shadow-sm relative overflow-hidden">
                                ...
                                <div className="flex justify-between items-start mb-6">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className={`text-xs font-bold uppercase px-2 py-1 rounded-full inline-block mt-1 ${pickupOrder.status === 'delivered' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                                            }`}>
                        ...
                        {loadingDrivers ? (
                            ...
                            drivers.map(driver => (
                                <div
                                    ...
                                    className={`p-4 rounded-xl border-2 cursor-pointer transition-all flex items-center justify-between ${selectedDriverId === driver.id
    ...
    return (
        <div className={`rounded-xl shadow-sm border border-stone-200 ${theme.bg} flex flex-col overflow-hidden relative group hover:shadow-md transition-all`}>
            ...
            <div className="p-3 pl-5 border-b border-stone-100 flex justify-between items-start">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] md:text-sm font-black text-stone-900 bg-stone-100 px-2 py-0.5 rounded-md">#{order.dailyNumber}</span>
                        {order.type === 'takeout' || order.type === 'delivery' ? (
                            <span className={`text-[9px] md:text-[10px] font-black uppercase px-2 py-0.5 rounded flex items-center gap-1 ${order.type === 'delivery' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'
                                }`}>
                                {order.type === 'delivery' ? '🛵 LIVRAISON' : '👜 EMPORTER'}
                            </span>
                        ) : (
                            <span className="text-xs md:text-sm font-bold text-stone-700 bg-stone-100 px-2 rounded">{order.table?.name || '?'}</span>
                        )}
                    </div>
                    <div className="text-[10px] md:text-xs text-stone-400 font-medium flex items-center gap-1.5">
                        <Clock className="w-3 h-3" />
                        {new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        {order.clientName && <span className="text-stone-600 font-bold truncate max-w-[80px] md:max-w-none">• {order.clientName}</span>}
                    </div>
                </div>
            </div>
                ...
                {order.items.map((item: any, i: number) => (
                    <div key={i} className="flex justify-between items-start text-sm leading-tight">
                        <div className="text-stone-900">
                            <span className="font-black mr-2 bg-stone-100 px-1 md:px-1.5 rounded-md text-stone-600">{item.quantity}</span>
                            <span className="font-bold text-[13px] md:text-sm">{item.menuItem?.name}</span>
